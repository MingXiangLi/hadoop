...
public class DataSetLockManager implements LockManager<AutoCloseLock> {
  ...
  private class TrackLog {
    ...
    public TrackLog(String threadName) {
  ...
  public DataSetLockManager(Configuration conf) {
    this.isFair = conf.getBoolean(DFSConfigKeys.DFS_DATANODE_BLOCKPOOL_LOCK_FAIR,
        DFSConfigKeys.DFS_DATANODE_BLOCKPOOL_LOCK_FAIR_DEFAULT);
    this.openLockTrace = conf.getBoolean(DFSConfigKeys.DFS_DATANODE_BLOCKPOOL_LOCK_TRACE,
        DFSConfigKeys.DFS_DATANODE_BLOCKPOOL_LOCK_TRACE_DEFAULT);
    LOG.info("Volume lock version");
  }
  ...
  private AutoCloseLock getReadLock(LockLevel level, String... resources) {
    ...
    if (lock == null) {
      LOG.error("Ignore this error during dn restart: Not existing readLock " + lockName);
      LOG.error(StringUtils.getStackTrace(Thread.currentThread()));
